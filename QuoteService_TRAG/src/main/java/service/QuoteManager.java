...
public class QuoteManager {
    ...
    public Quote obtenerCotizacion(Long id)
            throws BusinessException, PersistenceException {
    ...
    public List<Quote> obtenerTodasCotizaciones()
            throws PersistenceException {

        return quoteDAO.getAllQuotes();
    }

    private CustomerSummaryResponse validarCliente(long clienteId)
        ...
        try {
            ...
            if (!resp.getExists()) {
                ...
                throw new BusinessException(
                        "Cliente no encontrado o inactivo con ID: "
                        + clienteId);
            }
            ...
            throw new BusinessException(
                    "Error gRPC con CustomerAndVehicleService: "
                    + e.getMessage());
        }
    }

    private VehicleSummaryResponse validarVehiculo(long vehiculoId)
            throws BusinessException {
        ...
        try {
            ...
            if (!resp.getExists()) {
                ...
                throw new BusinessException(
                        "Vehículo no encontrado o inactivo con ID: "
                        + vehiculoId);
            }
            ...
            throw new BusinessException(
                    "Error gRPC con CustomerAndVehicleService (vehículo): "
                    + e.getMessage());
        }
    }

    private ServiceDetailResponse obtenerServicio(long serviceId)
            throws BusinessException {
        ...
        try {
            ...
            throw new BusinessException(
                    "Error gRPC con InventoryAndServiceService: "
                    + e.getMessage());
        }
    }
    ...
    private Quote construirQuote(
        ...
        for (SupplyItemResponse s : supplies) {
            ...
            qs.setPrice(
                    BigDecimal.valueOf(s.getSuggestedCost()));
    ...
    public void shutdown() {
        ...
        if (customerChannel != null
                && !customerChannel.isShutdown()) {

            customerChannel.shutdown();
        }

        if (inventoryChannel != null
                && !inventoryChannel.isShutdown()) {
